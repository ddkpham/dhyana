...
function SearchUser() {
  ...
  const searchResults = () => { 
    if (hasSearched && contactList.length > 0) {
      var results = [];
      const ceiling = Math.min(firstIndexPresented+numResultsToShow, contactList.length)
  ...
  const nextButton = () => {
    if (contactList.length > (firstIndexPresented + numResultsToShow)) {
      return (
        <Button variant="outlined" color="primary" onClick={() => setFirstIndexPresented(firstIndexPresented+numResultsToShow)}>
          Next
        </Button>
      )
    } else {
